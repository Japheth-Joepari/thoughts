...
@section('content')
    ...
    <div class="p-6 sm:p-10 ">
        <div class="flex flex-wrap  md:flex-row justify-between">
            <div class="">
                <h2 class="text-4xl font-semibold mb-2">Categories</h2>
                <h2 class="text-gray-600 ml-0.5">View all categories here ...</h2>
            </div>
                    ...
                    <tr>
                        ...
                        <td class="border px-4 py-2">
                            <a class="text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800 px-3"
                                ...
                                <button id="trash-post-{{ $category->id }}"
                                ...
                                <div
                                    ...
                                    <div>
                                        <form action="{{ route('categories.destroy', $category) }}"
                                            ...
                                            @method('delete')
                                            <div class="mb-4">
                                                <p class="text-sm text-gray-600">Are you sure you want to move this category to trash ?</p>
                                            </div>
                                        </form>
                                        <div class="flex justify-end">
                                            <button type="submit"
                                                class="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded-lg"
                                                id="yes-button-{{ $category->id }}">Yes</button>
                                            <button
                                                class="bg-gray-300 hover:bg-gray-400 text-gray-800 font-bold py-2 px-4 rounded-lg ml-3"
                                                id="no-button-{{ $category->id }}">No</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </td>
                        <td class="border px-4 py-2 text-sm leading-5 font-medium text-gray-900">{{ $category->user->name }}
                        </td>
                        <td class="border px-4 py-2 text-sm leading-5 font-medium text-gray-900">
                            {{ $category->posts_count }} posts</td>
                    </tr>
                @endforeach
            </tbody>
        </table>
    @else
        <p class="text-gray-600 text-center my-10">No categories found. Try creating some ...</p>
    @endif
