                        ...
                        <article class="overflow-hidden rounded-lg  shadow-lg">
                            ...
                            <footer class="flex-col  items-center justify-between leading-none p-2 md:p-4">
                                ...
                                <div class="flex-row justify-start w-full">
                                    <span
                                        class="px-2 text-xs leading-5 font-semibold rounded-full bg-red-100 text-green-800">{{ $post->category->name }}</span>
                                    @foreach ($post->tags as $postTag)
                                        <span
                                            class="px-2  text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800">{{ $postTag->name }}</span>
                                    @endforeach
                                </div>
                                ...
                                <div class="flex justify-start">
                                    <a href="{{ route('posts.edit', $post) }}"
                                        class=" py-2 text-xs font-medium text-gray-500 hover:text-gray-700">Edit</a>

                                    <button type="submit" id="trash-post-{{ $post->id }}"
                                        class="px-4 py-2 text-xs font-medium text-gray-500 hover:text-gray-700 trash-post">Delete</button>

                                    <form method="POST" action="{{ route('posts.toggleFeatured', $post) }}">
                                        @csrf
                                        @method('PUT')
                                        <button type="submit"
                                            class="px-4 py-2 text-xs font-medium text-gray-500 hover:text-gray-700 trash-post"
